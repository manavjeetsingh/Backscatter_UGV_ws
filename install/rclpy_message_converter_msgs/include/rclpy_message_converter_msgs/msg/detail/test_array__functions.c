  ...
  rclpy_message_converter_msgs__msg__TestArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rclpy_message_converter_msgs__msg__TestArray * data =
      (rclpy_message_converter_msgs__msg__TestArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rclpy_message_converter_msgs__msg__TestArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rclpy_message_converter_msgs__msg__TestArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
